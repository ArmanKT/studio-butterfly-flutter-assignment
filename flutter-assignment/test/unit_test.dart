...
void main() {
  group('Money Arithmetic Precision Tests', () {
    test('Double precision error demonstrating Bug #3', () {
      ...
      const double doubleSum = rate * multiplier;
    ...
    test(
      'Money minor units precision math guarantees exact billing results',
      () {
  ...
  group('Model Serialization Tests', () {
    test(
      'SmsMessage parses decimal strings correctly without float conversion',
      () {
        final json = {
          'messageId': 'SM123',
          'recipient': '+4915*****78',
  ...
  group('Network Repository & Mock Client Integration Tests', () {
    ...
    setUp(() {
      ...
      repository = NetworkSmsRepository(
        client: mockClient,
    ...
    test(
      'Successful SMS send returns response model matching API contract',
      () async {
    ...
    test(
      'Rate-limiting 429 status code parses Retry-After header and throws RateLimitException',
      () async {
    ...
    test(
      'Invalid phone number E.164 formats throws ValidationException',
      () async {
        expect(
          () => repository.sendSms(
            tenantId: tenantId,
    ...
    test(
      'Tenant switcher token expired forces TokenExpiredException',
      () async {
        expect(
          () => repository.sendSms(
            tenantId: tenantId,
